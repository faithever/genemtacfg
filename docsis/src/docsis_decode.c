...
find_symbol_by_code_and_pid (unsigned char code, unsigned int pid)
{
 ...
 for ( i=0; i<NUM_IDENTIFIERS; i++) {
        if (global_symtable[i].docsis_code == code && global_symtable[i].parent_id == pid) {
                //printf("id(%d) docsis code(%d), symtable[%d] pid(%d) and name(%s)\n", global_symtable[i].id, code, i, pid, global_symtable[i].sym_ident);
...
#define TLV_LEN_SIZE 1
   ...
   while ( (unsigned int) (cp - tlvbuf) < buflen ) {
      ...
      if (is_mta) {
         if (cp[0] == 64) {
            printf("Find MTA current symbol\n");
      ...
      if (current_symbol == NULL) {
         ...
         if (1)
         {
            int i;
            char* tmp = (char*)cp;
            for (i=0;i < (int)(1+tlv_llen+tlv_vlen);i++)
            {
               printf("[0x%02x]", tmp[i]);
            }
         }
         printf("Calling decode function with current symbol (%s), tlv_llen(%d) tlv_vlen(%d)))\n", current_symbol->sym_ident, tlv_llen, (int)tlv_vlen);
         */
         current_symbol->decode_func (cp+1+tlv_llen, current_symbol, tlv_vlen );
      }
#ifdef DEBUG
